...
class Log(commands.Cog):
    ...
    async def on_message_delete(self, ctx):
        ...
        if len(result) == 0:
            ...
            async for entry in ctx.guild.audit_logs(action=discord.AuditLogAction.message_delete):
                xd = f"{entry.user.mention} (**{entry.user.name}**)"
            embed = discord_webhook.DiscordEmbed(title="Üzenet törlése", description=f"{ctx.author.mention} (**{ctx.author}**) üzenetét törölték a {ctx.channel.mention} csatornában.", color=0xFF9900)
            embed.add_embed_field(name="Üzenet", value=cleanmsg)
            embed.add_embed_field(name="Törölte", value=xd)
    ...
    async def on_message_edit(self, before, after):
        ...
        if len(result) == 0:
            ...
            if not result[0][0] == before.guild.id: return
                ...
                webhook.add_embed(embed)
                webhook.execute()
            if before.channel.pins != after.channel.pins:
                embed = discord_webhook.DiscordEmbed(title="Üzenet kitűzése", color=szin)
                embed.add_embed_field(name="Üzenet", value=after.content)
                embed.add_embed_field(name="Csatorna", value=after.channel.mention)
                embed.add_embed_field(name="Jelenlegi kitűzések száma", value=len(after.channel.pins))
                webhookurl = f"https://discord.com/api/webhooks/{result[0][2]}"
                webhook = discord_webhook.DiscordWebhook(url=webhookurl, embed=embed)
                webhookurl = f"https://discord.com/api/webhooks/{result[0][2]}"
                webhook = discord_webhook.DiscordWebhook(url=webhookurl, embed=embed)
    ...
    async def on_guild_role_update(self, before, after):
        ...
        if len(result) == 0:
            ...
            if before.permissions != after.permissions:
                a1 = str(before.permissions).replace("<Permissions value=", "")
                a2 = str(a1).replace(">", "")
                b1 = str(after.permissions).replace("<Permissions value=", "")
                b2 = str(b1).replace(">", "")
                embed.add_embed_field(name="Régi jogai (számokban)", value=str(b2))
                embed.add_embed_field(name="Új jogai (számokban)", value=str(a2))
                webhookurl = f"https://discord.com/api/webhooks/{result[0][2]}"
                webhook = discord_webhook.DiscordWebhook(url=webhookurl, embed=embed)
                webhook.add_embed(embed)
                webhook.execute()
    ...
    async def on_bulk_message_delete(self, messages):
        cursor = db.cursor()
        cursor.execute(f"SELECT * FROM `log` WHERE guild_id={int(messages.guild.id)}")
        result = cursor.fetchall()
        if len(result) == 0:
            return
        else:
            if not result[0][0] == messages.guild.id: return 
            xd = " "
