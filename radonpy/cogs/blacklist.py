...
class Blacklist(commands.Cog):
    ...
    @commands.command()
    async def blreport(self, ctx, user, *,reason):
        try: user = await commands.UserConverter().convert(ctx, user)
        except: await ctx.send("<:radon_x:856423841667743804> A felhasználó nem található!"); return
        msg = await ctx.send("Jelentésed elküldése folyamatban...")
        reportChannel = self.client.get_channel(861596296019836938)
        cursor = db.cursor()
        ...
        if (len(result) == 0): await ctx.send("<:radon_x:811191514482212874> Hiba történt a jelentésed elküldése közben! Kérlek próbáld meg újra!")
        else:
            user_id = result[0][1]
            case_id = result[0][0]
            dbreason = result[0][3]
        ...
        with open("number.json", "w") as f: json.dump(number, f, indent=4)

    @commands.command()
    async def bldecline(self, ctx, id):
        if ctx.channel.id == 861596296019836938:
            msg = await ctx.send("Kérlek várj...")
            cursor = db.cursor()
            cursor.execute(f"SELECT * FROM blacklist_queue WHERE case_id={id}")
            result = cursor.fetchall()
            if (len(result) == 0): await msg.edit(content="Ezzel az ID-val nem található bejelentés")
            else:
