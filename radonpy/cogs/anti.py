    ...
    async def removebypass(self, ctx, member):
        if ctx.author.guild_permissions.administrator:
            ...
            if not len(result) == 0:
                ...
                db.commit()
                await ctx.send(f"Sikeresen eltávolítottam {member.mention} felhasználót a listáról. Mostantól ezen a szerveren az auto moderáció nem fogja ignorálni őt.")
            else:
