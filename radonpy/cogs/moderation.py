...
class Moderation(commands.Cog):
    ...
    @commands.command(usage=",clear [üzentek száma]", aliases=["purge"])
    ...
    @commands.command(usage=",dm [@felhasználó] [üzenet]")
    async def dm(self, ctx, user, *, message):
        user = await commands.UserConverter().convert(ctx, user)
        try: 
            await user.send(f"Üzenetet kaptál **{ctx.author}**-tól/től: `{message}`")
        except: 
            await ctx.reply("Nem tudok ennek a felhasználónak üzenetet küldeni!")
        await ctx.message.delete()

    @commands.command(usage=",warn [felhasználó] (indok)", aliases=["figyelmeztet", "figyelmeztetés", "figyelmeztetes"])
    async def warn(self, ctx, member, *, reason="Nincs indok megadva."):
        if ctx.author.guild_permissions.kick_members:
            try: member = await commands.MemberConverter().convert(ctx, member)
            except: 
                embed1 = discord.Embed(description="Nem található ilyen felhasználó a szerveren! <:radon_x:856423841667743804>", color=0xff9900, timestamp=datetime.datetime.utcnow())
...
#            embed.add_field(name="Figyelmeztetés ID", value=currentid)
    ...
    async def kick(self, ctx, member, *, reason="Nincs indok megadva."):
        if ctx.author.guild_permissions.kick_members:
            try: member = await commands.MemberConverter().convert(ctx, member)
                ...
                embed1 = discord.Embed(description="Nem található ilyen felhasználó a szerveren! 0", color=0xff9900, timestamp=datetime.datetime.utcnow())
                embed1.set_author(name="Hiba!", icon_url=self.client.user.avatar_url)
                embed1.set_footer(text=f"{ctx.author.name} × Hiba", icon_url=ctx.author.avatar_url)
                await ctx.reply(embed=embed1, mention_author=False)
                return
    ...
    async def ban(self, ctx, member, *, reason="Nincs indok megadva"):
    ...
    async def tempban(self, ctx, member, ido, *, reason="Nincs indok megadva"):
        if ctx.author.guild_permissions.ban_members:
            try: ido=convert(ido)
    ...
    async def softban(self, ctx, member, *, reason="Nincs indok megadva"):
        if ctx.author.guild_permissions.ban_members:
            try: member = await commands.MemberConverter().convert(ctx, member)
            except: 
                embed1 = discord.Embed(description="Nem található ilyen felhasználó a szerveren! <:radon_x:856423841667743804>", color=0xff9900, timestamp=datetime.datetime.utcnow())
    ...
    async def unban(self, ctx, user):
        if ctx.author.guild_permissions.ban_members == False: 
            ...
            if user.isdigit() == False: 
                lista = user.split('#')
                if len(lista) != 2: 
                    embed = discord.Embed(description="Nem található ilyen felhasználó a kitiltottak listáján! <:radon_x:856423841667743804>", color=0xff9900, timestamp=datetime.datetime.utcnow())
    ...
    async def addrole(self, ctx, user, *, role):   
        if ctx.author.guild_permissions.manage_roles == False: 
            ...
            try: user = await commands.MemberConverter().convert(ctx, user)
                ...
                embed = discord.Embed(description="Nem található ilyen felhasználó a szerveren! <:radon_x:856423841667743804>", color=0xff9900, timestamp=datetime.datetime.utcnow())
                embed.set_author(name=f"Hiba × {ctx.author.name}#{ctx.author.discriminator}", icon_url=ctx.author.avatar_url)
                await ctx.reply(embed=embed, mention_author=False)
                return
            try: role = await commands.RoleConverter().convert(ctx, role)
            except:
                embed = discord.Embed(description="Nem található ilyen rang a szerveren! <:radon_x:856423841667743804>", color=0xff9900, timestamp=datetime.datetime.utcnow())
                embed.set_author(name=f"Hiba × {ctx.author.name}#{ctx.author.discriminator}", icon_url=ctx.author.avatar_url)
                await ctx.reply(embed = embed, mention_author=False)
                return
            if ctx.author.id == user.id: 
                embed = discord.Embed(description="Nem adhatsz magadnak rangot! <:radon_x:856423841667743804>", color=0xff9900, timestamp=datetime.datetime.utcnow())
                embed.set_author(name=f"Hiba × {ctx.author.name}#{ctx.author.discriminator}", icon_url=ctx.author.avatar_url)
                await ctx.reply(embed=embed, mention_author=False)
                return
            elif ctx.author.guild.owner == False:
                if ctx.author.top_role < role: 
                    embed = discord.Embed(description="Ez a rang magasabb mint a te rangod, ezért nem adhatod oda másnak! <:radon_x:856423841667743804>", color=0xff9900, timestamp=datetime.datetime.utcnow())
                    embed.set_author(name=f"Hiba × {ctx.author.name}#{ctx.author.discriminator}", icon_url=ctx.author.avatar_url)
                    await ctx.reply(embed=embed, mention_author=False)
                    return
            else:
            ...
            if bot.guild_permissions.manage_roles == False: 
                embed = discord.Embed(description="Nincs jogosultságom a felhasználó kezelésére! <:radon_x:856423841667743804>", color=0xff9900, timestamp=datetime.datetime.utcnow())
                embed.set_author(name=f"Hiba × {ctx.author.name}#{ctx.author.discriminator}", icon_url=ctx.author.avatar_url)
                await ctx.reply(embed=embed, mention_author=False)
                return
    ...
    async def removerole(self, ctx, user, *, role):   
        if ctx.author.guild_permissions.manage_roles == False: 
            ...
            try: role = await commands.RoleConverter().convert(ctx, role)
                ...
                return
            if ctx.author.id == user.id: 
                embed = discord.Embed(description="Nem vehetsz le magadról rangot!<:radon_x:856423841667743804>", color=0xff9900, timestamp=datetime.datetime.utcnow())
                embed.set_author(name=f"Hiba × {ctx.author.name}#{ctx.author.discriminator}", icon_url=ctx.author.avatar_url)
                await ctx.reply(embed=embed, mention_author=False)
                return
            elif ctx.author.guild.owner == False:
                if ctx.author.top_role < role: 
                    embed = discord.Embed(description="Ez a rang magasabb mint a te rangod, ezért nem veheted le másról!<:radon_x:856423841667743804>", color=0xff9900, timestamp=datetime.datetime.utcnow())
                    embed.set_author(name=f"Hiba × {ctx.author.name}#{ctx.author.discriminator}", icon_url=ctx.author.avatar_url)
                    await ctx.reply(embed=embed, mention_author=False)
                    return
            else:
