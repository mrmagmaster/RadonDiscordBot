...
class Economy(commands.Cog):
    ...
    async def slots(self, ctx):
        msg = await ctx.reply("A gép pörög!", mention_author=False)
        slots = [ ":banana:",":watermelon:",":strawberry:",":apple:",":pear:",":cherries:" ]
        slot1 = random.choice(slots)
        slot2 = random.choice(slots)
        slot3 = random.choice(slots)
        embed = discord.Embed(title="Slots", color=0xff9900, timestamp=datetime.datetime.utcnow())
        await msg.edit(embed=embed)
        await asyncio.sleep(1)
        embed.add_field(name="Első slot", value=slot1, inline=True)
        await msg.edit(embed=embed)
        await asyncio.sleep(1.1)
        embed.add_field(name="Második slot", value=slot2, inline=True)
        await msg.edit(embed=embed)
        await asyncio.sleep(1.3)
        embed.add_field(name="Harmadik slot", value=slot3, inline=True)
        await msg.edit(embed=embed)
        if slot1 == slot2 and slot1 == slot3 and slot2 == slot3:
            embed.set_author(name="Nyertél!")
            await msg.edit(embed=embed)
        else:
            embed.set_author(name="Vesztettél!")
            await msg.edit(embed=embed)
    ...
    @commands.command(aliases=["dep","betesz","bankbatesz"], usage=",deposit [összeg]")
    ...
    @commands.command(aliases=['with','kivesz','készpénzbetesz'], usage=",withdraw [összes]")
    ...
    @commands.command(usage=",buy [azonosító]")
    async def buy(self, ctx, idd: int):
        try:
            cursor = db.cursor()
            cursor.execute(f"SELECT * FROM economy_keszpenz WHERE user_id = {ctx.author.id}")
            penztarca = cursor.fetchall()
            cursor.execute(f"SELECT * FROM shop WHERE guild_id = {ctx.guild.id} AND azonosito = {idd}")
